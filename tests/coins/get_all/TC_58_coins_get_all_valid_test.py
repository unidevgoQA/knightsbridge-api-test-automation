...
def test_coins_get_all_valid():
    coin_type_endpoint = "coin/get-all?isActive={}&coinType={}".format(coins_get_all_params['isActive'],
                                                                       coins_get_all_params['coinType'])
    coin_api = Api(coin_type_endpoint)
    result = coin_api.get_request(payload=coins_get_all_params, headers=admin_headers_with_token)
